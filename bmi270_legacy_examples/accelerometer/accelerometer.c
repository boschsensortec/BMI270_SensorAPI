...
int main(void)
{
    ...
    if (rslt == BMI2_OK)
    {
        ...
        if (rslt == BMI2_OK)
        {
             ...
             */
            rslt = bmi270_legacy_sensor_enable(&sensor_list, 1, &dev);
            bmi2_error_codes_print_result(rslt);

            printf("Accel raw data and gravity data in m/s^2 \n");
            ...
            while (indx <= limit)
            {
                ...
                if (int_status & BMI2_ACC_DRDY_INT_MASK)
                {
                    ...
                    x = lsb_to_mps2(sensor_data.acc.x, 2, dev.resolution);
                    y = lsb_to_mps2(sensor_data.acc.y, 2, dev.resolution);
                    z = lsb_to_mps2(sensor_data.acc.z, 2, dev.resolution);
