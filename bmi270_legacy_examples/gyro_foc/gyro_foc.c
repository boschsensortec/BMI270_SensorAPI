...
int main(void)
{
    ...
    for (iteration = 0; iteration < 2; iteration++)
    {
        ...
        /* Enable gyroscope */
        rslt = bmi270_legacy_sensor_enable(&sens_list, 1, &dev);
        bmi2_error_codes_print_result(rslt);

        rslt = perform_gyro_foc_test(&dev);

        if ((iteration == 0) && (rslt == BMI2_E_OUT_OF_RANGE))
        {
        ...
        else if ((iteration > 0) && (rslt == BMI2_OK))
        {
        ...
        else if ((iteration > 0) && (rslt == BMI2_E_OUT_OF_RANGE))
        {
...
static int8_t perform_gyro_foc_test(struct bmi2_dev *bmi2_dev)
{
    ...
    /* Enable gyroscope */
    rslt = bmi270_legacy_sensor_enable(&sens_list, 1, bmi2_dev);
    bmi2_error_codes_print_result(rslt);
