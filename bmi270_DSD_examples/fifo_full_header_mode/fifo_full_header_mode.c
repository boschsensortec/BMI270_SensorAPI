...
#define BMI270_DSD_FIFO_RAW_DATA_BUFFER_SIZE  UINT16_C(2048)

/*! Length of data to be read from FIFO. */
#define BMI270_DSD_FIFO_RAW_DATA_USER_LENGTH  UINT16_C(2048)
...
uint16_t fifo_buffer_size = BMI270_DSD_FIFO_RAW_DATA_BUFFER_SIZE + SENSORTIME_OVERHEAD_BYTE;
 ...
 */
uint8_t fifo_data[BMI270_DSD_FIFO_RAW_DATA_BUFFER_SIZE + SENSORTIME_OVERHEAD_BYTE];
...
int main(void)
{
    ...
    fifoframe.length = BMI270_DSD_FIFO_RAW_DATA_USER_LENGTH + SENSORTIME_OVERHEAD_BYTE;
