...
int main(void)
{
     ...
     */
    rslt = bmi2_sensor_enable(sensor_list, BMI2_N_SENSE_COUNT_3, &bmi);
    bmi2_error_codes_print_result(rslt);
    ...
    printf("\nMAGNETOMETER, ACCEL, AND GYRO DATA IN DATA MODE\n");
    ...
    if (bmm.chip_id == BMM150_CHIP_ID)
    {
        ...
        printf("\n%8s\t %10s\t %9s\t %9s\t %9s\t %10s\t %10s\t %10s\t %7s\t %7s\t %7s\n",
               "Data_set",
               "Status_Reg",
               "Acc_ms2_X",
               "Acc_ms2_Y",
        ...
        for (;;)
        {
                ...
                (sens_data.status & BMI2_DRDY_GYR))
            {
                ...
                acc_x = lsb_to_mps2(sens_data.acc.x, BMI2_GET_RANGE_VAL(BMI2_ACC_RANGE_2G), bmi.resolution);
                acc_y = lsb_to_mps2(sens_data.acc.y, BMI2_GET_RANGE_VAL(BMI2_ACC_RANGE_2G), bmi.resolution);
                acc_z = lsb_to_mps2(sens_data.acc.z, BMI2_GET_RANGE_VAL(BMI2_ACC_RANGE_2G), bmi.resolution);

                /* Converting lsb to degree per second for 16 bit gyro at 2000dps range. */
                gyr_x = lsb_to_dps(sens_data.gyr.x, BMI2_GET_DPS_VAL(BMI2_GYR_RANGE_2000), bmi.resolution);
                gyr_y = lsb_to_dps(sens_data.gyr.y, BMI2_GET_DPS_VAL(BMI2_GYR_RANGE_2000), bmi.resolution);
                gyr_z = lsb_to_dps(sens_data.gyr.z, BMI2_GET_DPS_VAL(BMI2_GYR_RANGE_2000), bmi.resolution);
                ...
                printf("%8d\t %8s%x\t %9.2f\t %9.2f\t %9.2f\t %10.2f\t %10.2f\t %10.2f\t %7ld\t %7ld\t %7ld\n",
                       count,
                       "0x",
                       sens_data.status,
                       acc_x,
...
static void bmi2_accel_gyro_conf(struct bmi2_dev *bmi)
{
    ...
    rslt = bmi2_get_sensor_config(config, BMI2_N_SENSE_COUNT_2, bmi);
    bmi2_error_codes_print_result(rslt);
...
static void bmi2_aux_conf(uint8_t mag_odr, struct bmm150_dev *bmm, struct bmi2_dev *bmi)
{
    ...
    else
    {
        ...
        rslt = bmi2_set_sensor_config(&bmi2_conf, BMI2_N_SENSE_COUNT_1, bmi);
        bmi2_error_codes_print_result(rslt);

        rslt = bmi2_sensor_disable(&bmi2_sens, BMI2_N_SENSE_COUNT_1, bmi);
        bmi2_error_codes_print_result(rslt);
    }
}
